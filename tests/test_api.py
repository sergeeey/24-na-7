...
def test_ingest_audio_sync_smoke_returns_transcription(tmp_path):
    wav_header = bytes(
        [
    ...
    client = TestClient(app)
    with patch("src.api.routers.ingest.process_audio_bytes") as mock_process:
        ...
        files = {"file": ("test.wav", wav_header, "audio/wav")}
        resp = client.post("/ingest/audio?sync=true", files=files)

    assert resp.status_code == 200
    payload = resp.json()
    assert payload["status"] == "transcribed"
    assert payload["transcription"]["text"] == "hello world"
    assert payload["transcription"]["language"] == "en"
    assert mock_process.await_args.kwargs["transcribe_now"] is True
    assert mock_process.await_args.kwargs["run_enrichment"] is False
