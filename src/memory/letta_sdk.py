...
class LettaSDK:
    ...
    def __init__(self, api_key: Optional[str] = None, cache_enabled: bool = True):
        ...
        self.memory_dir = settings.STORAGE_PATH
        self.legacy_memory_dir = Path(".cursor/memory")
    ...
    def _store_local(self, key: str, value: Any, memory_type: str) -> bool:
        ...
        try:
            ...
            if memory_type == "core":
                file_path = self.memory_dir / "core_memory.json"
            else:
                session_dir = self.memory_dir / "session_memory"
    ...
    def _get_local(self, key: str, memory_type: str) -> Optional[Any]:
        """Получение из локального хранилища с fallback на legacy .cursor."""
        try:
            if memory_type == "core":
                file_path = self.memory_dir / "core_memory.json"
                legacy_path = self.legacy_memory_dir / "core_memory.json"
            else:
                file_path = self.memory_dir / "session_memory" / f"{key}.json"
                legacy_path = self.legacy_memory_dir / "session_memory" / f"{key}.json"

            source_path = file_path if file_path.exists() else legacy_path
