...
def is_speech_energy_filter(
    ...
    high_freq_threshold: float = 0.45,
) -> Tuple[bool, dict]:
    """
    ...
    if LIBROSA_AVAILABLE:
        ...
        try:
            ...
            is_speech = speech_ratio >= energy_threshold and high_freq_ratio <= high_freq_threshold
            ...
            metrics = {
                ...
                "is_speech": is_speech,
                "reason": "energy_filter"
                if is_speech
                else f"speech_ratio={speech_ratio:.2f}, high_freq={high_freq_ratio:.2f}",
            }
            ...
            return _numpy_energy_filter(
                audio, sample_rate, speech_band_low, speech_band_high, energy_threshold
            )
    else:
...
def _numpy_energy_filter(
    ...
    try:
        ...
        # ПОЧЕМУ 0.45: телефонные микрофоны дают больше высоких частот чем десктопные,
        # при 0.3 реальная речь отфильтровывалась как "не речь"
        is_speech = speech_ratio >= energy_threshold and high_freq_ratio <= 0.45
...
class SpeechFilter:
    ...
    def filter_segment(
        self, audio: np.ndarray, segment_info: Optional[dict] = None
    ) -> Tuple[bool, dict]:
        """
