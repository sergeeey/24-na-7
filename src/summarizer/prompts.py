...
ВАЖНО про topics:
- topics — это 2-3 КОНКРЕТНЫХ ключевых слова из текста (существительные/названия).
- ПРАВИЛЬНО: ["шкаф", "ремонт"], ["Макдональдс", "мороженое"], ["приложение", "баг"]
- НЕПРАВИЛЬНО: ["текст содержит описание"], ["фрагмент"], ["высказывание"], ["говорящий выражает"]
- НЕ описывай текст. Извлеки ИЗ текста конкретные предметы разговора.

ВАЖНО про speakers:
- Если в тексте упоминается имя человека (Катерина, Марат, мама, папа) — ВСЕГДА добавь в speakers.
- Даже если человек не говорит, а о нём говорят ("Катерина просила") — добавь.
...
{{
    ...
    "output": {{
        ...
        "topics": ["конкретное_слово1", "конкретное_слово2"],
        "actions": ["действие1"],
        "decisions": ["решение1"],
        "speakers": ["имя1"],
