    ...
    with ToolTimer() as timer:
        try:
            ...
            if not results:
                ...
                # Фильтрация по дате
                results = [
                    r for r in raw_results if _in_range(r.get("created_at", ""), start_iso, end_iso)
                ]
            ...
            any_tasks = any(_safe_json_list(r.get("tasks")) for r in results)
        ...
        try:
            ...
            for row in rows:
                ...
                if topic_filter and not any(topic_filter in value.lower() for value in topics):
                    continue
                if participant_filter and not any(
                    participant_filter in value.lower() for value in participants
                ):
                    continue
        ...
        try:
            ...
            if not row or row["status"] != "ready":
                ...
                try:
                    ...
                    if not digest_data.get("total_recordings") and not digest_data.get(
                        "summary_text"
                    ):
    ...
    try:
        rows = db.fetchall(
            ...
            WHERE created_at BETWEEN ? AND ?
              AND length(text) > 10
              AND text NOT LIKE '%субтитры%'
              AND text NOT LIKE '%спасибо за просмотр%'
              AND text NOT LIKE '%подпишитесь%'
            ORDER BY created_at DESC
            LIMIT ?
            """,
