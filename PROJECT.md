...
> **Версия:** 0.5.2-beta | **Дата:** 2026-03-11 | **Статус:** Production (Beta)
...
**Версия/статус:** текущий release snapshot — `0.5.2-beta`. На проде уже живут truth layer, `episodes`, `day_threads`, `long_threads`, `/query/threads`, `pipeline-status` и `pipeline-trends`. Импорт app, GET /health и актуальный regression subset проходят.

### Release Snapshot — 0.5.2-beta
...
|--------|------|-----------|
| 0.5.2-beta | 2026-03-11 | Release discipline + confidence hardening: semantic golden regression fixture for the memory pipeline, benchmark smoke script for `episode -> day_thread -> long_thread`, continuity scoring fix for trusted topics, cleaned release snapshot/docs surface. |
